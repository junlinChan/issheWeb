<template>
    <div class="user-list-container">
      ...
      <el-table
          ...
          <el-table-column
              ...
              <template slot-scope="scope">
                ...
                <el-button type="text" size="small" @click="changeMobileClick(scope.row)">修改信息</el-button>
                 <el-button type="text" size="small" @click="changePwdClick(scope.row)">重置密码</el-button>
      ...
      <el-dialog title="重置密码" :visible.sync="changePwdFormVisible">
          <el-form ref="changePwdForm" :model="changePwdForm" label-width="70px" :rules="addUserRule">
              <el-form-item label="登陆名">
                  <el-input v-model="changePwdForm.username" placeholder="请输入登陆名" disabled></el-input>
              </el-form-item>
              <el-form-item label="姓名">
                  <el-input v-model="changePwdForm.name" placeholder="请输入姓名" disabled></el-input>
              </el-form-item>
              <el-form-item label="密码" prop="password">
                  <el-input v-model="changePwdForm.password" type="password" placeholder="请输入密码"></el-input>
              </el-form-item>
          </el-form>
          <div slot="footer" class="dialog-footer">
              <el-button @click="changePwdFormVisible = false">取 消</el-button>
              <el-button type="primary" @click="submitChangePwdClick">确 定</el-button>
          </div>
      </el-dialog>
    </div>
</template>
...
<script>
import { getUserList, deleteUser, addUser, updateUser, getGroupsList, getUserGroupsList, updateUserGroupsList, updatePwd } from '@/api/users'
...
export default {
  ...
  methods: {
    ...
    submitChangePwdClick() {
      this.$refs['changePwdForm'].validate((valid) => {
        if (!valid) {
          return
        }
        updatePwd(this.changePwdForm.id, { username: this.changePwdForm.username, name: this.changePwdForm.name, password: this.changePwdForm.password }).then(() => {
